-- Enable proper authentication and configure password security
...
DROP POLICY IF EXISTS "Only admins can manage EmailJS settings" ON public.emailjs_settings;
CREATE POLICY "Authenticated admins can manage EmailJS settings" 
...
CREATE POLICY "Authenticated admins can manage licenses" 
ON public.licenses 
FOR ALL 
TO authenticated
USING (public.is_admin())
...
DROP POLICY IF EXISTS "Only admins can modify plan validity settings" ON public.plan_validity_settings;
CREATE POLICY "Authenticated admins can modify plan validity settings" 
...
DROP POLICY IF EXISTS "Only admins can modify SEO settings" ON public.seo_settings;
CREATE POLICY "Authenticated admins can modify SEO settings" 
...
CREATE POLICY "Authenticated admins can modify website settings" 
...
DROP POLICY IF EXISTS "Only admins can access audit logs" ON public.admin_audit_log;
CREATE POLICY "Authenticated admins can access audit logs" 
...
-- Storage policies - require authentication for admin operations
...
CREATE POLICY "Authenticated admin users can delete files" 
ON storage.objects 
FOR DELETE 
TO authenticated
USING (public.is_admin());

CREATE POLICY "Authenticated admin users can update files" 
ON storage.objects 
FOR UPDATE 
TO authenticated
USING (public.is_admin());

CREATE POLICY "Authenticated admin users can view files" 
ON storage.objects 
FOR SELECT 
TO authenticated
USING (public.is_admin());

CREATE POLICY "Authenticated admin users can insert files" 
ON storage.objects 
FOR INSERT 
TO authenticated
WITH CHECK (public.is_admin());
